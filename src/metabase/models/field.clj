...
(def has-field-values-options
  ...
  ;; `:search` or `:none`. This is the default state for Fields not marked `auto_list`. Admins cannot explicitly mark
  ;; a Field as `has_field_values` `nil`. This value is also subject to automatically change in the future if the
  ;; values of a Field change in such a way that it can now be marked `auto_list`. Fields marked `nil` do *not* have
  ;; FieldValues objects.
  ;;
  #{;; The other automatically option. Automatically marked as a 'List' Field based on cardinality and other factors
    ;; during sync. Store a FieldValues object; use the List Widget. If this Field goes over the distinct value
    ;; threshold in a future sync, the Field should get switched back to `has_field_values = nil`.
    :auto_list
    ;;
    ...
    ;; Widget. Unlike `auto_list`, if this Field grows past the normal cardinality constraints in the future, it will
...
(defn- infer-has-field-values
  "Determine the value of `has_field_values` we should return for a `Field`."
  [{has-field-values :has_field_values, :as field}]
  (or
   ;; if `has_field_values` is set in the DB, use that value; but if it's `auto_list`, return the value as `list` to
   ;; avoid confusing FE code
   (when has-field-values
     (if (= (keyword has-field-values) :auto_list)
       :list
       has-field-values))
